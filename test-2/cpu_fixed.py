#!/usr/bin/env python3
# ============================================================
# CPU-OPTIMIZED Parkinson's EEG - FIXED FOR CLASS IMBALANCE
# ============================================================
...
MODEL_SAVE_DIR = r"C:\projects\NCF-lite\test-2\models"
...
# ============================================================
# AUGMENTATIONS - REDUCED to prevent overfitting
# ============================================================
class Augment:
    def __init__(self, p_noise=0.2, p_shift=0.2, p_chdrop=0.1, p_banddrop=0.15,
                 noise_std=0.01, max_shift_frac=0.05, chdrop_frac=0.05):
...
class WindowDataset(torch.utils.data.Dataset):
    def __init__(self, records, canon_chs, extract_windows_fn, training=False, augment=None):
        ...
        if np.std(self.sfreqs) > 1e-6:
            print("[WARN] Sampling rate varies across recordings.")
...
class SubjectBagDataset(torch.utils.data.Dataset):
    def __init__(self, records, canon_chs, extract_windows_fn, bag_size=6, training=False, augment=None):
        self.training = training
        self.augment = augment
...
class NeuroConvFormerLite(nn.Module):
    def __init__(self, n_ch, n_time, d_model=64, n_heads=4, n_layers=2, dropout=0.3):
        super().__init__()
...
# ============================================================
# METRICS - ENHANCED with confusion matrix
# ============================================================
...
def train_one_fold(fold, train_df, val_df, canon_chs, extract_windows_fn,
    ...
    mil_batch_size = 4  # Increased from 2

    # BALANCED sampling (moderate weighting)
    ...
    # MODERATE weighting (not too aggressive)
    w0 = 1.5  # Boost minority class moderately
    w1 = 1.0
    ...
    # LARGER model
    model = NeuroConvFormerLite(n_ch=n_ch, n_time=n_time, d_model=64, n_heads=4, n_layers=2, dropout=0.3).to(DEVICE)
    ...
    if n_params > 250_000:
        print(f"[WARNING] Params {n_params} > 250k budget, but needed for better performance")

    # SIMPLE weighted CE (no Focal)
    ...
    w0_ce = 1.5  # Boost minority class
    w1_ce = 1.0
    class_w = torch.tensor([w0_ce, w1_ce], dtype=torch.float32, device=DEVICE)

    crit = nn.CrossEntropyLoss(weight=class_w)  # Simple CE instead of Focal
    ...
    for ep in range(1, epochs + 1):
        ...
        for xb_bag, yb, _ in pbar:
            ...
            z = model.forward_features(xb_flat)
            z = z.view(Bsubj, BAG, -1)
            z_subj = pool(z)
            logits_subj = model.head(z_subj)
        ...
        # Print confusion matrix to debug
...
if __name__ == "__main__":
    ...
    def discover_eeg_files(root_dir):
        ...
        if len(files) == 0:
            raise FileNotFoundError(f"No EEG files found in {root_dir}")
        return files
    ...
    if len(file_rows) == 0:
        raise RuntimeError("No files matched with labels.")
    ...
    if len(canon) < 4:
        raise RuntimeError(f"Canonical EEG intersection too small: {len(canon)} channels")
    ...
    def extract_windows_from_raw(raw, canon_chs, win_sec=10.0, overlap=0.50):
        ...
        if len(picks) != len(canon_chs):
            raise RuntimeError("Canonical channel mismatch.")
    ...
    if os.path.exists(MODEL_SAVE_DIR):
        ...
        summary = {
            'overall_metrics': {
                'window': {k: {'mean': float(v[0]), 'std': float(v[1])} for k, v in win_sum.items()},
                'subject': {k: {'mean': float(v[0]), 'std': float(v[1])} for k, v in subj_sum.items()}
            },
            'fold_results': [
                {
                    'fold': i,
                    'window': {k: float(w[k]) for k in ['acc', 'bacc', 'prec', 'rec', 'f1', 'auc']},
                    'subject': {k: float(s[k]) for k in ['acc', 'bacc', 'prec', 'rec', 'f1', 'auc']}
                }
                for i, (w, s) in enumerate(fold_results, start=1)
            ],
            'num_folds': len(fold_results),
            'canonical_channels': canon,
            'num_channels': len(canon)
        }

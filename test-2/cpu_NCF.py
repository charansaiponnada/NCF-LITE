#!/usr/bin/env python3
# ============================================================
# CPU-OPTIMIZED Parkinson's Resting-State EEG
# Optimized for Intel i5-12450H CPU execution
# ============================================================
...
MODEL_SAVE_DIR = r"C:\projects\NCF-lite\test-2\models"  # Directory to save models

# -----------------------
# GLOBALS - CPU OPTIMIZED
# -----------------------
SEED = 42
...
class SubjectBagDataset(torch.utils.data.Dataset):
    def __init__(self, records, canon_chs, extract_windows_fn, bag_size=4, training=False, augment=None):
        self.training = training
        self.augment = augment
...
# ============================================================
# MODEL - CPU OPTIMIZED (Reduced complexity)
# ============================================================
...
class NeuroConvFormerLite(nn.Module):
    def __init__(self, n_ch, n_time, d_model=48, n_heads=2, n_layers=1, dropout=0.2):
        super().__init__()
...
def train_one_fold(fold, train_df, val_df, canon_chs, extract_windows_fn,
                   epochs=20, batch_size=8, lr=2e-3, weight_decay=1e-3,
    ...
    model = NeuroConvFormerLite(n_ch=n_ch, n_time=n_time, d_model=48, n_heads=2, n_layers=1, dropout=0.25).to(DEVICE)
    pool = MILPool(d_model=48).to(DEVICE)
    ...
    for ep in range(1, epochs + 1):
        ...
        for xb_bag, yb, _ in pbar:
            ...
            # Direct forward pass (no AMP for CPU)

...
# ============================================================
# SAVE FINAL TRAINED MODEL (AFTER CV SUMMARY)
# ============================================================
# NOTE:
# We save the LAST trained model instance.
# For external validation, this is the standard way.
# (Optionally: retrain once on full dataset if desired.)

save_trained_model(
    model=model,
    canon_channels=canon,
    win_summary=win_sum,
    subj_summary=subj_sum,
    save_dir="pretrained",
    model_name="ncf_ds004584"
)
...
def save_trained_model(
    ...
    Saves:
      ...
      - metadata required for reproducibility & external validation
    """
    ...
    # ---- build metadata from ACTUAL training ----
    metadata = {
        "dataset": "OpenNeuro ds004584",
        "task": "Parkinsons Disease EEG Classification",
        "training_type": "Subject-wise Stratified 5-Fold CV",
        "architecture": "NeuroConvFormer-Lite",
        "num_parameters": sum(p.numel() for p in model.parameters() if p.requires_grad),
        "window_sec": WIN_SEC,
        "overlap": OVERLAP,
        "normalization": "per-window channel-wise z-score",
        "canonical_eeg_channels": canon_channels,
        "metrics": {
            "WINDOW_LEVEL": {
                k: f"{win_summary[k][0]:.4f}±{win_summary[k][1]:.4f}"
                for k in win_summary
            },
            "SUBJECT_LEVEL": {
                k: f"{subj_summary[k][0]:.4f}±{subj_summary[k][1]:.4f}"
                for k in subj_summary
            }
        }
    }
